...
if ($row = pg_fetch_assoc($result)) {
    ...
    $id_pengguna = $row['id']; // Ambil id pengguna dari tabel pengguna

    // Ambil semua post dari user ini
    $sql_post = "SELECT content FROM post WHERE id_pengguna = $1 ORDER BY id DESC";
    ...
    <div class="wrapper">
        ...
        <div class="main-content">
            ...
            <div class="posts post-container">
                <?php while ($post = pg_fetch_assoc($result_post)) : ?>
                    <div class="post">
                        <img src="<?php echo htmlspecialchars($fotoProfil); ?>" class="post-pic">
                        <div class="post-content">
                            <p><strong><?php echo htmlspecialchars($namaLengkap); ?></strong> @<?php echo htmlspecialchars($usernameFix); ?> • just now</p>
    ...
    submitPost.addEventListener('click', function() {
        ...
        .then(result => {
            if (result.success) {
                ...
                window.location.reload(); // Reload untuk tampilkan post terbaru
            } else {
